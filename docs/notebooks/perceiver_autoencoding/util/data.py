...
def load_video(path, max_frames=0, resize=(224, 224)):
    ...
    try:
        ...
        cap.release()
    return jnp.array(frames) / 255.


def load_audio(path):
    sample_rate, audio = wavfile.read(path)
    if audio.dtype == jnp.int16:
        audio = audio.astype(jnp.float32) / 2**15
    elif audio.dtype != jnp.float32:
        raise ValueError(
            "Unexpected datatype. Sound samples are expected to lie in [-1, 1]."
        )
    return audio

def output_positions(chunk_shape, chunk_index, n_chunks, n_frames=1):
    seqlen = reduce(operator.mul, chunk_shape[1:-1])
    chunk_size = seqlen // n_frames // n_chunks
    return jnp.arange(chunk_size * chunk_index, chunk_size * (chunk_index + 1))

# Kinetics 700 Classes
LABELS = ["abseiling", "acting in play", "adjusting glasses", "air drumming", 
